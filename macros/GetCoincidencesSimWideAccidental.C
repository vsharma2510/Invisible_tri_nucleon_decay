...
void PlotCoincidenceTime(vector<pair<double,double>> pair_vector)
{
  ...
  coincidence_plot->Draw();
  TFile* output_file = TFile::Open("../output_plots/127In_coincidence_output_wide_timing_acc.root","RECREATE");
  output_file->cd();
      ...
      if(chain_num==temp_chain_num)
	{
	  ...
	  if(multiplicity==2)
	      ...
	      if(prompt_event_found)
		{
		  ...
		  if(total_energy>delayed_low && total_energy<delayed_high && time-prompt_time>720 && time-prompt_time<1400)
		    {
